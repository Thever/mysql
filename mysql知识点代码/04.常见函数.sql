...
select employee_id,last_name,salary,salary*1.2 '工资提高百分之 20%后的结果' FROM employees;
...
# 统计1的个数--统计行数，新增一行带1，必定统计正确
select COUNT(1) FROM employees;

# 同上，统计常量就是行数
select COUNT('jojo') FROM employees;
...
SELECT MAX(salary) 最大值, MIN(salary) 最小值, AVG(salary) 平均值, SUM(salary) 总和 FROM employees ORDER BY job_id ASC; 
